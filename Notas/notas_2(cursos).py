...
def cursos_proximos(num_alumno):
    curso1 = linecache.getline(notas_cursos[0],num_alumno).strip()
    curso2 = linecache.getline(notas_cursos[1],num_alumno).strip()
    curso3 = linecache.getline(notas_cursos[2],num_alumno).strip()
    curso4 = linecache.getline(notas_cursos[3],num_alumno).strip()
    curso5 = linecache.getline(notas_cursos[4],num_alumno).strip()
    curso6 = linecache.getline(notas_cursos[5],num_alumno).strip()
    curso7 = linecache.getline(notas_cursos[6],num_alumno).strip()

    cursos_a_llevar = ""
    #print(curso5)
    if(int(curso1)>10):
        cursos_a_llevar += tercero_2[3] + ",\n"
    if(int(curso2)>10):
        cursos_a_llevar += segundo_2[4] + ",\n"
##        cursos_a_llevar += tercero_1[5] + ",\n"
    if(int(curso3)>10):
        cursos_a_llevar += segundo_2[0] + ",\n"
##        cursos_a_llevar += tercero_1[2] + ",\n"
    if(int(curso5)>10):
        cursos_a_llevar += tercero_2[4] + ",\n"
    if(int(curso6)>10):
        cursos_a_llevar += quinto_1[4] + ",\n"
    #print(curso2)

    return cursos_a_llevar
    
print("Cursos posibles a llevar: " + cursos_proximos(1))
print("Cursos posibles a llevar: " + cursos_proximos(9))

print("Cursos posibles a llevar: " + cursos_proximos(8))

print("Cursos posibles a llevar: " + cursos_proximos(7))

print("Cursos posibles a llevar: " + cursos_proximos(6))

print("Cursos posibles a llevar: " + cursos_proximos(5))
...
def pdf_notas_final(curso):
    ...
    for i in range (0,39):
        ...
        if(curso==0):
            promedio_final[i]=(int(con_1)*0.09)+(int(con_2)*0.09)+(int(con_3)*0.15)+((int(par_1)*0.2)+(int(par_2)*0.2)+(int(par_3)*0.27))
        elif(curso==1):
            promedio_final[i]=(int(con_1)*0.18)+(int(con_2)*0.18)+(int(con_3)*0.24)+((int(par_1)*0.12)+(int(par_2)*0.12)+(int(par_3)*0.16))
        elif(curso==2):
            promedio_final[i]=(int(con_1)*0.2)+(int(con_2)*0.2)+(int(con_3)*0.27)+((int(par_1)*0.09)+(int(par_2)*0.09)+(int(par_3)*0.15))
        elif(curso==3):
            promedio_final[i]=(int(con_1)*0.1)+(int(con_2)*0.1)+(int(con_3)*0.3)+((int(par_1)*0.1)+(int(par_2)*0.1)+(int(par_3)*0.3))
        elif(curso==4):
            promedio_final[i]=(int(con_1)*0.06)+(int(con_2)*0.06)+(int(con_3)*0.08)+((int(par_1)*0.24)+(int(par_2)*0.24)+(int(par_3)*0.32))
        elif(curso==5):
            promedio_final[i]=(int(con_1)*0.09)+(int(con_2)*0.09)+(int(con_3)*0.15)+((int(par_1)*0.2)+(int(par_2)*0.2)+(int(par_3)*0.27))
        elif(curso==6):
            promedio_final[i]=(int(con_1)*0.2)+(int(con_2)*0.2)+(int(con_3)*0.2)+((int(par_1)*0.1)+(int(par_2)*0.15)+(int(par_3)*0.15))
...
def reporte(conta):
    ...
    mywindow.geometry("900x430")
    imag.place(x=700,y=120)
    subt.place(x=700,y=70)
    subt_2.place_forget()
    cursos_futuros.place_forget()

def reporte_2(conta):

    mywindow.geometry("1000x430")
    #imag.place(x=700,y=120)
    imag.place_forget()
    subt.place_forget()
    subt_2.place(x=700,y=120)
    cursos_futuros.place(x=700,y=120)
    cursos_futuros['text']=cursos_proximos(contador4)
...
def imprimir(comp):#funcion imprimir
    global contador2,contador3,contador4#indicas variable global 
    if(comp==1):
        ...
        etiqueta4['text']=lista_alum[contador4]
        contador4+=1
        mywindow.geometry("750x430")
        imag.place_forget()
        subt.place_forget()
        subt_2.place_forget()
        cursos_futuros.place_forget()
        if(contador4-1==39):
            texto3="Fin..."
            etiqueta4['text']==texto3
            switch(submit_btn_sig)
...
def ingresar_notas(boton):
    ...
    reporte_label.place(x = 550, y = 120)#ubicacion 
    etiqueta4.place(x = 500, y = 185)#ubicacion
    submit_btn_sig.place(x = 530, y = 250)#ubicacion
    submit_btn_report.place(x = 530, y = 300)#ubicacion
    submit_btn_report_2.place(x = 530, y = 340)#ubicacion
...
reporte_label = Label(text = "REPORTE:", bg = "#00FFFF",font=("",13))
subt = Label(mywindow,text = "Cursos del siguiente semestre" , font = ("Cambria", 11), bg = "#56CD63", fg = "black", width = "25", height = "1")
subt_2 = Label(mywindow,text = "Cursos de otros semestres" , font = ("Cambria", 11), bg = "#56CD63", fg = "black", width = "25", height = "1")

etiqueta4=Label(text=nombre, font= ("",12))

submit_btn_sig = Button(mywindow,text = "SIGUIENTE",command =lambda:[imprimir(3)], width = "11", height = "2",bg = "#00CD63")
submit_btn_report = Button(mywindow,text = "MOSTRAR",command =lambda:[reporte(contador4)], width = "11", height = "2",bg = "#00CD63")
submit_btn_report_2 = Button(mywindow,text = "MOSTRAR2",command =lambda:[reporte_2(contador4)], width = "11", height = "2",bg = "#00CD63")


cursos_photo = ImageTk.PhotoImage(Image.open("cursos.png"))
imag = Label(mywindow, image=cursos_photo)

cursos_futuros = Label(mywindow,text = "", font = ("Cambria", 12), bg = "#56CD63", fg = "black", width =33, height =8, anchor="e")
